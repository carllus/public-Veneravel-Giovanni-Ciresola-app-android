...
class QueryParser {
    ...
    /* renamed from: tq */
    private TokenQueue f61tq;

    private QueryParser(String str) {
        this.query = str;
        this.f61tq = new TokenQueue(str);
    }
    ...
    public Evaluator parse() {
        this.f61tq.consumeWhitespace();
        if (this.f61tq.matchesAny(combinators)) {
            this.evals.add(new StructuralEvaluator.Root());
            combinator(this.f61tq.consume());
        } else {
            findElements();
        }
        while (!this.f61tq.isEmpty()) {
            boolean consumeWhitespace = this.f61tq.consumeWhitespace();
            if (this.f61tq.matchesAny(combinators)) {
                combinator(this.f61tq.consume());
    ...
    private void combinator(char r11) {
        /*
            r10 = this;
            org.jsoup.parser.TokenQueue r0 = r10.f61tq
            ...
            boolean r5 = r1 instanceof org.jsoup.select.CombiningEvaluator.C0649Or
            if (r5 == 0) goto L_0x003a
            if (r11 == r2) goto L_0x003a
            r5 = r1
            org.jsoup.select.CombiningEvaluator$Or r5 = (org.jsoup.select.CombiningEvaluator.C0649Or) r5
        ...
        L_0x0096:
            ...
            boolean r11 = r1 instanceof org.jsoup.select.CombiningEvaluator.C0649Or
            if (r11 == 0) goto L_0x00a3
            org.jsoup.select.CombiningEvaluator$Or r1 = (org.jsoup.select.CombiningEvaluator.C0649Or) r1
            r1.add(r0)
            r11 = r1
            goto L_0x00ae
        L_0x00a3:
        ...
        L_0x00ae:
            ...
            r0 = r5
            org.jsoup.select.CombiningEvaluator$Or r0 = (org.jsoup.select.CombiningEvaluator.C0649Or) r0
    ...
    private String consumeSubQuery() {
        ...
        while (!this.f61tq.isEmpty()) {
            if (this.f61tq.matches("(")) {
                sb.append("(").append(this.f61tq.chompBalanced('(', ')')).append(")");
            } else if (this.f61tq.matches("[")) {
                sb.append("[").append(this.f61tq.chompBalanced('[', ']')).append("]");
            } else if (this.f61tq.matchesAny(combinators)) {
                break;
            } else {
                sb.append(this.f61tq.consume());
            }
        }
        return sb.toString();
    }
    ...
    private void findElements() {
        if (this.f61tq.matchChomp("#")) {
            byId();
        } else if (this.f61tq.matchChomp(".")) {
            byClass();
        } else if (this.f61tq.matchesWord()) {
            byTag();
        } else if (this.f61tq.matches("[")) {
            byAttribute();
        } else if (this.f61tq.matchChomp("*")) {
            allElements();
        } else if (this.f61tq.matchChomp(":lt(")) {
            indexLessThan();
        } else if (this.f61tq.matchChomp(":gt(")) {
            indexGreaterThan();
        } else if (this.f61tq.matchChomp(":eq(")) {
            indexEquals();
        } else if (this.f61tq.matches(":has(")) {
            has();
        } else if (this.f61tq.matches(":contains(")) {
            contains(false);
        } else if (this.f61tq.matches(":containsOwn(")) {
            contains(true);
        } else if (this.f61tq.matches(":matches(")) {
            matches(false);
        } else if (this.f61tq.matches(":matchesOwn(")) {
            matches(true);
        } else if (this.f61tq.matches(":not(")) {
            not();
        } else if (this.f61tq.matchChomp(":nth-child(")) {
            cssNthChild(false, false);
        } else if (this.f61tq.matchChomp(":nth-last-child(")) {
            cssNthChild(true, false);
        } else if (this.f61tq.matchChomp(":nth-of-type(")) {
            cssNthChild(false, true);
        } else if (this.f61tq.matchChomp(":nth-last-of-type(")) {
            cssNthChild(true, true);
        } else if (this.f61tq.matchChomp(":first-child")) {
            this.evals.add(new Evaluator.IsFirstChild());
        } else if (this.f61tq.matchChomp(":last-child")) {
            this.evals.add(new Evaluator.IsLastChild());
        } else if (this.f61tq.matchChomp(":first-of-type")) {
            this.evals.add(new Evaluator.IsFirstOfType());
        } else if (this.f61tq.matchChomp(":last-of-type")) {
            this.evals.add(new Evaluator.IsLastOfType());
        } else if (this.f61tq.matchChomp(":only-child")) {
            this.evals.add(new Evaluator.IsOnlyChild());
        } else if (this.f61tq.matchChomp(":only-of-type")) {
            this.evals.add(new Evaluator.IsOnlyOfType());
        } else if (this.f61tq.matchChomp(":empty")) {
            this.evals.add(new Evaluator.IsEmpty());
        } else if (this.f61tq.matchChomp(":root")) {
            this.evals.add(new Evaluator.IsRoot());
        } else {
            throw new Selector.SelectorParseException("Could not parse query '%s': unexpected token at '%s'", this.query, this.f61tq.remainder());
        }
    }

    private void byId() {
        String consumeCssIdentifier = this.f61tq.consumeCssIdentifier();
        Validate.notEmpty(consumeCssIdentifier);
        this.evals.add(new Evaluator.C0650Id(consumeCssIdentifier));
    }
    ...
    private void byTag() {
        String consumeElementSelector = this.f61tq.consumeElementSelector();
    ...
    private void byAttribute() {
        TokenQueue tokenQueue = new TokenQueue(this.f61tq.chompBalanced('[', ']'));
    ...
    private void cssNthChild(boolean z, boolean z2) {
        String lowerCase = this.f61tq.chompTo(")").trim().toLowerCase();
    ...
    private int consumeIndex() {
        String trim = this.f61tq.chompTo(")").trim();
        Validate.isTrue(StringUtil.isNumeric(trim), "Index must be numeric");
        return Integer.parseInt(trim);
    }

    private void has() {
        this.f61tq.consume(":has");
        String chompBalanced = this.f61tq.chompBalanced('(', ')');
        Validate.notEmpty(chompBalanced, ":has(el) subselect must not be empty");
        this.evals.add(new StructuralEvaluator.Has(parse(chompBalanced)));
    }

    private void contains(boolean z) {
        this.f61tq.consume(z ? ":containsOwn" : ":contains");
        String unescape = TokenQueue.unescape(this.f61tq.chompBalanced('(', ')'));
    ...
    private void matches(boolean z) {
        this.f61tq.consume(z ? ":matchesOwn" : ":matches");
        String chompBalanced = this.f61tq.chompBalanced('(', ')');
